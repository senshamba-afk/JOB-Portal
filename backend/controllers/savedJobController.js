...
exports.saveJob = async (req, res) =>{
    try {
        ...
        if(exists) return res.status(400).json({message:"Job already unsaved"});

        const savedJob = await SavedJob.findOneAndDelete({job: req.params.jobId, jobseeker: req.user._id});
